...
def create_all_directories(ROOT_DIR, PERMUTE_TRAIN_TEST):
    ...
    if not os.path.exists(ROOT_DIR+"Descriptive_Statistics"+os.path.sep+"All_Features_vs_Target"):
        os.makedirs(ROOT_DIR+"Descriptive_Statistics"+os.path.sep+"All_Features_vs_Target")
    ...
    __methods__ = ["LinRegr", "NLRegr", "XGBoost", "RF", "ANNBN", "DANN", "DecisionTrees"]
    for __method__ in __methods__:
        create_ml_dir(ROOT_DIR,__method__)
    if not os.path.exists(ROOT_DIR+"ML_Models"+os.sep+"DecisionTrees"+os.sep+"VariousDepthTrees"+os.sep):
        os.makedirs(ROOT_DIR+"ML_Models"+os.sep+"DecisionTrees"+os.sep+"VariousDepthTrees"+os.sep)    
    ...
    if not os.path.exists(ROOT_DIR+"LaTeX_report"):
        os.makedirs(ROOT_DIR+"LaTeX_report")
...
def make_features_and_target_categorical_if_so(df, target_name, ROOT_DIR, save_processed):
    ...
    if len(cat_cols)>0:
        ...
        # It creates 2 columns, one per category. Multicoliearity will remove the second, later.
        ...
        if target_name == cat_cols[-1]:
            ...
            df = pd.concat([df_num, onehot_encoded], axis=1)
        else:
            df_num = df.select_dtypes(include=['number'])
            df = pd.concat([df_num.iloc[:,:-1], onehot_encoded, df_num.iloc[:,-1]], axis=1)

        print(df.head())
        
        if save_processed:
            if not os.path.exists(ROOT_DIR+os.sep+"ProcessedExcel"):
                os.makedirs(ROOT_DIR+os.sep+"ProcessedExcel")
            df.to_excel(ROOT_DIR+os.sep+"ProcessedExcel"+os.sep+"ProcessedExcel.xlsx", index=False)
...
def cdf_pdf_plot(varVals,var_,path_):
    bin = 20
    #bin = int(len(var1)/10)
    _pdf_,_range_ = calc_pdf(varVals,bin)
    fig, ax = plt.subplots(ncols=1, dpi=200)
    ax.set_xlabel(var_)
    ...
    iso = argsort(varVals)
    cdf = arange(1,len(varVals)+1)/(len(varVals)+1)##CHECK!
    ax1.plot(varVals[iso],cdf,color='gray',marker='.',linewidth=0.5,markersize=1.5)
...
def split_tr_vl(obs,___perc_cv___,nof_folds,PERMUTE_TRAIN_TEST):
    ...
    # It calculates the number of observations to be used for validation (obs_fold) 
    # by multiplying ___perc_cv___ with the total number of observations (obs). 
    # The remaining observations are assigned to the training set (obs_valid = obs - obs_fold).
    obs_fold = int(math.ceil(___perc_cv___*obs))
    obs_valid = obs - obs_fold

    # A variable i is initialized to 1. If PERMUTE_TRAIN_TEST is true, it shuffles 
    # the range of observations using a random seed i and selects the first obs_valid observations. 
    # Otherwise, it selects the first obs_valid observations directly. This represents the initial validation set.
    # True is usefull for time-series data, when we want to predict the future, trained on past data.
    i = 1
    if PERMUTE_TRAIN_TEST:
    ...
    # The validation set indices (obs_scanned) are stored in a list called vl_inds.
    vl_inds = [obs_scanned]
    __restart__ = False
    while i < nof_folds:
        i += 1
        obs_to_add = []
        # If the number of remaining observations is less than or equal to 
        # obs_valid + obs % obs_valid, it means there are not enough observations 
        # left to form a complete validation set. 
        if obs - len(obs_scanned) <= obs_valid + obs % obs_valid:
            __restart__ = True
            obs_to_add = list(set(range(obs)) - set(obs_scanned))
        # If there are enough remaining observations, it calculates ii as the 
        # set difference between the full range of observations and the ones already scanned. 
        else:
            __restart__ = False
            ii = list(set(range(obs)) - set(obs_scanned))
            # Then, if PERMUTE_TRAIN_TEST is true, it shuffles ii using a random 
            # seed i and selects the first obs_valid observations. Otherwise, it selects 
            # the first obs_valid observations from ii directly.
            if PERMUTE_TRAIN_TEST:
        ...
        # If the flag __restart__ is True and i is less than nof_folds, 
        # we start with a new obs_scanned, and append it to vl_inds 
    ...
    # check whether there are overlapings
    ...
    # A list called tr_inds is created to store the indices of the training sets. 
    # It iterates nof_folds times, and for each iteration, it calculates the set difference 
    # between the full range of observations and the corresponding validation set indices.
    tr_inds = []
    ...
    # Two empty lists, lvalid and ltrain, are created to store the indices of all validation and
    # training samples, respectively. They are populated by concatenating the indices 
    # from vl_inds and tr_inds for each fold.
    lvalid = []
    ltrain = []
    for i in range(nof_folds):
        lvalid += vl_inds[i]
    ...
    # The function prints the total number of validation and training samples, 
    # as well as the number of unique samples in each set.
...
def gather_all_sensitivity_curves(Xtr, ytr, features_names, target_name, sens_methods, ROOT_DIR):
    delta_y = ytr.max()-ytr.min()
    try:
                        ...
                        for dir1 in dirs1:
                            if dir1.startswith("Sensitivity_Analysis"):
                                for root2, dirs2, files2 in os.walk(os.path.join(ROOT_DIR, "ML_Models", dir, dir1)):
                                    for file in files2:
                                        if file.startswith("sens_med_all") and (file[13:-5] in sens_methods):
                                            ...
                                            s_i = (df[features_names[i]].max()-df[features_names[i]].min())/delta_y
                                            sens_all[i] += s_i
                                            
                                            print(f"Sensitivity Curves for {file} {features_names[i]} OK:",s_i)
            sens_all[i] /= len(sens_methods)
            plt.ylabel(target_name)
            plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0.)
            plt.tight_layout()
            s_i_all = str(int(100*sens_all[i]))
            if sens_all[i]<0.1:
                s_i_all = "0" + s_i_all
            plt.savefig(ROOT_DIR + "ML_Models" + os.sep + "ALL_Sensitivity" + os.sep + s_i_all + f"_{features_names[i]}_sensitivity_curve.png")
            plt.close()
        ...
        for i, v in enumerate(sens_all[iso]):
            plt.text(v + 0.0, i + 0.0, str(round(v, 3)), color='black')
        plt.xlabel("Sensitivity")
        plt.ylabel("Features")
        plt.tight_layout()
        plt.savefig(ROOT_DIR + "ML_Models" + os.sep + "ALL_Sensitivity" + os.sep + "All_Sensitivity.png")
        plt.close()
        
        with open(ROOT_DIR + "ML_Models" + os.sep + "ALL_Sensitivity" + os.sep + "All_Sensitivity.txt", "w") as f:
            for i in iso[::-1]:
                f.write(features_names[i] + "," + str(sens_all[i]) + "\n")
    except Exception as ex1:
        print(ex1)
...
def gather_all_ML_metrics(ROOT_DIR):    
    ...
    df_latex =  df_all.iloc[:, :7]
    column_format = '|l|' + '|'.join(['c'] * 6) +'|'
    latex_table = df_latex.to_latex(column_format=column_format, float_format='%.3g', index=False)
    ...
    with open(ROOT_DIR + "ML_Models" + os.sep + "All_ML_metrics.txt", "w") as f:
        f.write("\\begin{table}[H]\n")
        f.write("\\centering\n")
        f.write("\\caption{Accuracy Metrics for all Machie Learning Models in the Train and Test Sets}\n")
        f.write("\\label{tab:All_ML_metrics}\n")                                
        f.write(latex_table)
        f.write("\\end{table}\n")
    ...
    df_all["Model"].to_csv(ROOT_DIR + "ML_Models" + os.sep + "all_models_sorted_by_mae.txt", index=False)
...
def delete_files_except_xlsx(root_folders):
        ...
        if os.path.isdir(folder):
            for root, dirs, files in os.walk(folder):
                for dir in dirs:
                    ...
                    print(sub_folder,"Deleted.")
        else:
            print(f"Folder '{folder}' does not exist.")
            
            
def make_many_dirs_for_many_outputs(PATH_MAIN_EXCEL,INITIAL_COLS_TO_DELETE,LAST_COLS_TARGETS):
    
    s_all = []
    for file in os.listdir(PATH_MAIN_EXCEL):
        if file.endswith(".xlsx"):
            s_all.append(PATH_MAIN_EXCEL + file)
    ...
    df = df.iloc[:, INITIAL_COLS_TO_DELETE:]

    num_columns = len(df.columns)
    last_columns = df.columns[num_columns-LAST_COLS_TARGETS:num_columns]

    ROOT_DIR_ALL = []
    for column in last_columns:
        new_df = pd.concat([df.iloc[:, :num_columns-LAST_COLS_TARGETS], df[column]], axis=1)
        if not os.path.exists(PATH_MAIN_EXCEL+os.sep+column):
            os.makedirs(PATH_MAIN_EXCEL+os.sep+column)
        new_df.to_excel(PATH_MAIN_EXCEL+os.sep+column+os.sep+column+".xlsx",index=False)
        ROOT_DIR_ALL.append(PATH_MAIN_EXCEL+os.sep+column)
        print(column)
