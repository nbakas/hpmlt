...
def augment_train_dataset(Xtr,ytr,max_nof_samples,noise_factor):
    Xtr_all = Xtr.copy()
    ytr_all = ytr.copy()
    while Xtr_all.shape[0]<max_nof_samples:
        Xr = 1+noise_factor*(rand(Xtr.shape[0],Xtr.shape[1])-1/2)
        Xrr = Xr*Xtr
        XrrV = reshape(Xrr,(Xtr.shape[0]*Xtr.shape[1],))
        XtrV = reshape(Xtr,(Xtr.shape[0]*Xtr.shape[1],))
        Xtr_all = concatenate((Xtr_all,Xrr),axis=0)
        ytr_all = concatenate((ytr_all,ytr),axis=0)
        print(iter,Xtr_all.shape,corrcoef(XrrV,XtrV)[0,1])
    return Xtr_all, ytr_all
...
def do_ANNBN(Xtr, Xte, ytr, yte, features_names, target_name, list_run, PERMUTE_TRAIN_TEST, LOGISTIC_REGR, ROOT_DIR):
    global w_all, V
    ...
    try:
        ...
        for i in list_run:    
            try:
                ...
                for fold in range(nof_folds):
                    ...
                    accTr = pearsonr(ytr[tr_inds[fold]], pred_tr).correlation
                    if isnan(accTr):
                        accTr = 0
                    acc_tr += accTr
                    
                    accVl = pearsonr(ytr[vl_inds[fold]], pred_vl).correlation
                    if isnan(accVl):
                        accVl = 0
                    acc_vl += accVl

                    accTe = pearsonr(yte, pred_te).correlation
                    if isnan(accTe):
                        accTe = 0
                    acc_te += accTe
                ...
                print(datetime.datetime.now().strftime("%H:%M:%S"),"CombNeurons:",i,". Rank:",ki,
        ...
        neurons = imax.copy()
        ytr, min_y, max_y = normalize_y(ytr)
        layer1, w_all, ii_all, neurons, idx = compute_w(Xtr,ytr,imax)
        w_all = array(w_all)
        ytr = denormalize_y(ytr, min_y, max_y)
        layer1_te = compute_layer1_te(Xte)
        ...
        plt.plot(V)
        plt.savefig(path_ + "V.png")
        plt.close()
        plt.hist(V)
        plt.savefig(path_ + "Vhist.png")
        plt.close()
